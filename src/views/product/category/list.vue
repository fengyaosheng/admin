  ...
  >
    ...
    <el-table-column label="操作" width="250" align="center">
      ...
      <template slot-scope="scope">
        <HintButton v-if="$hasBP('btn.Category.add')" title="添加子分类" :disabled="!scope.row.level" 
          size="mini" type="primary" icon="el-icon-plus"/>
        <HintButton v-if="$hasBP('btn.Category.update')" title="修改分类" 
          size="mini" type="primary" icon="el-icon-edit"/>
        <HintButton v-if="$hasBP('btn.Category.remove')" title="删除分类" 
