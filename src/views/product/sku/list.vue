  ...
  <el-card class="sku-list">
    <el-table border stripe :data="skuList" v-loading="loading">
      ...
      <el-table-column label="操作" width="250" align="center">
        <template slot-scope="{row}">
          <HintButton v-if="row.isSale == 0 && $hasBP('btn.Sku.updown')" title="上架" type="info" size="mini"
            icon="el-icon-top" @click="onSale(row.id)" />

          <HintButton v-if="row.isSale == 1 && $hasBP('btn.Sku.updown')" title="下架" type="success" size="mini"
            icon="el-icon-bottom" @click="cancelSale(row.id)"/>

          <HintButton title="修改" type="primary" size="mini"
            icon="el-icon-edit" @click="toUpdateSku(row.id)" v-if="$hasBP('btn.Sku.update')"/>

          <HintButton title="查看详情" type="info" size="mini"
            icon="el-icon-info" @click="showSkuInfo(row.id)" v-if="$hasBP('btn.Sku.detail')"/>

          <el-popconfirm v-if="$hasBP('btn.Sku.remove')" :title="`确定删除 ${row.skuName} 吗`" @onConfirm="deleteSku(row.id)">
    ...
    <el-drawer
      ...
      <el-row>
        ...
        <el-col :span="18">
          <h3 v-if="!skuInfo.skuSaleAttrValueList">无</h3>
          <el-tag v-else type="success" style="margin-right: 5px" v-for="value in skuInfo.skuSaleAttrValueList" :key="value.id">
