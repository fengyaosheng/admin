  ...
  <div>
    ...
    <el-card>
      <div v-show="!isShowSpuForm && !isShowSkuForm">
        <el-button type="primary" icon="el-icon-plus" :disabled="!category3Id" @click="showSpuAdd"
           v-if="$hasBP('btn.Spu.add')">添加SPU</el-button>
          ...
          <el-table-column label="操作">
            <template v-slot="{row, $index}">
              <hint-button title="添加SKU" type="primary" icon="el-icon-plus" size="mini" 
                @click="showSkuAdd(row)" v-if="$hasBP('btn.Spu.addsku')" />
              <hint-button title="修改SPU" type="primary" icon="el-icon-edit" size="mini" 
                @click="showSpuUpdate(row)" v-if="$hasBP('btn.Spu.update')" />
              <hint-button title="查看SKU列表" type="info" icon="el-icon-info" size="mini" 
                @click="showSkuList(row)" v-if="$hasBP('btn.Spu.skus')" />
              <el-popconfirm
                :title="`确定删除 ${row.spuName} 吗?`"
                @onConfirm="deleteSpu(row.id)"
                v-if="$hasBP('btn.Spu.delete')">
                <hint-button style="margin-left:10px" slot="reference" title="删除SPU" type="danger" icon="el-icon-delete" size="mini" />
              </el-popconfirm>
            </template>
          </el-table-column>
        </el-table>
