  ...
  <div>
    ...
    <el-card style="margin-top:20px;">
      <div v-show="isShowList">
        <el-button type="primary" icon="el-icon-plus" @click="showAdd" :disabled="!category3Id"
          v-if="$hasBP('btn.Attr.add')">添加属性</el-button>
        <el-table :data="attrs" border v-loading="loading">
          ...
          <el-table-column label="操作" width="150">
            <template slot-scope="{row, $index}">
              <hint-button title="修改属性" type="primary" icon="el-icon-edit" size="mini" @click="showUpdate(row)"
                v-if="$hasBP('btn.Attr.update')"></hint-button>

              <el-popconfirm :title="`确定删除 ${row.attrName} 吗?`" @onConfirm="deleteAttr(row.id)"
                v-if="$hasBP('btn.Attr.remove')">
                <HintButton style="margin-left:10px" slot="reference" title="删除属性" type="danger" icon="el-icon-delete"
      ...
      <div v-show="!isShowList">
        ...
        <el-table border style="margin: 20px 0" :data="attr.attrValueList">
          ...
          <el-table-column label="属性值名称">
            <template slot-scope="{row, $index}">
              <el-input :ref="$index" v-if="row.edit" v-model="row.valueName" placeholder="请输入名称" @blur="toList(row)" @keyup.enter.native="toList(row)" size="mini" />
          ...
          <el-table-column label="操作">
            <template slot-scope="{row, $index}">
              <el-popconfirm :title="`确定删除 ${row.valueName} 吗?`" @onConfirm="attr.attrValueList.splice($index, 1)">
                <hint-button style="margin-left:10px" slot="reference" title="删除" type="danger" icon="el-icon-delete"
                  size="mini"></hint-button>
              </el-popconfirm>
            </template>
          </el-table-column>
        </el-table>
