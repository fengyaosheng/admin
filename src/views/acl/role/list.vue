      ...
      <el-table-column label="操作" width="200" align="center">
        <template slot-scope="{row}">
          ...
          <HintButton size="mini" type="primary" icon="el-icon-check" title="确定" 
            @click="updateRole(row)" v-if="$hasBP('btn.Role.update') && row.edit"/>
          <HintButton size="mini" type="primary" icon="el-icon-edit" title="修改角色" 
            @click="row.edit= true" v-if="$hasBP('btn.Role.update') && !row.edit"/>
          <HintButton size="mini" type="danger" icon="el-icon-delete" title="删除角色"
            @click="removeRole(row)" v-if="$hasBP('btn.Role.remove')" />
        </template>
      </el-table-column>
    </el-table>
    ...
    */
    addRole(){
      ...
      this.$prompt('请输入新名称', '添加角色', {
        ...
        this.$API.role.save({roleName: value})
        .then(result => {
