<template>
  <el-card shadow="hover">
    <el-button slot="header" type="primary" icon="el-icon-plus" @click="showAdd" v-if="$hasBP('btn.Trademark.add')">添加</el-button>

    <el-table v-loading="loading" :data="trademarks" border>
    ...
    methods: {
      ...
      */
      remove(trademark) {
        ...
        this.$confirm(`确定删除 ${trademark.tmName} 吗?`, '提示', {
          ...
          if (this.trademarks.length === 1 && this.page > 1) {
            this.page -= 1
          }
          this.getTrademarks()
        }).catch((error) => { // 点击取消的回调
      ...
      addOrUpdate() {
        ...
        this.$refs['ruleForm'].validate(async (valid) => { // 校验完成的回调
          ...
          if (valid) {
            ...
            // 重新获取列表显示()
            // 哪一页?  更新显示当前页, 添加显示第一页
            this.page = trademark.id ? this.page : 1
            this.getTrademarks()
          } else { // 可以不用写
            console.log('校验不通过')
          }
      ...
      */
      async getTrademarks() {
        ...
        // 调用接口请求函数发获取列表数据的请求
        const result = await this.$API.trademark.getList(this.page, this.limit)
